...
def solution():
    ...
    br = [[False] * n for _ in range(h)] # 특정 지점 방문 여부 체크
...
def check(): # i번 세로선이 i번으로 도착하는지 확인
    for start in range(n): # 1번부터 N번 세로선(열)까지 검사
        ...
        for j in range(h): # 가로선(행) 이동
            if br[j][k]: # 가로선이 존재한다면
                k += 1 # 가로선 오른쪽으로 한칸 이동
            elif k > 0 and br[j][k - 1]: # 가로선이 왼쪽에 존재한다면
                k -= 1 # 가로선 왼쪽으로 한칸 이동
...
def dfs(cnt, x, y):
    ...
    for i in range(x, h):
        ...
        for j in range(k, n - 1): # 세로선(열)
            if not br[i][j] and not br[i][j + 1]: # 가로선을 놨을 때 오른쪽에 -가 존재하지 않는 경우
                if j > 0 and br[i][j - 1]: continue # 가로선을 놨을 때 왼쪽에 -가 존재할 경우 continue (--가 되면 안되기 때문)
                br[i][j] = True # 가로선 놓기
                dfs(cnt + 1, i, j + 2) # cnt 1 증가, 세로선 그대로, -- 이 되면 안되므로 가로선은 2증가
                br[i][j] = False # 가로선 없애기            
